...
namespace Kinect {
    ...
    [Serializable]
    public struct SerialKinectFrameData
    {
        private short[] depthData;
        private SerialColor32[] colorData;

        public SerialKinectFrameData(short[] depth, Color32[] color) {
            this.depthData = depth;
            this.colorData = new SerialColor32[color.Length];
            for (int i = 0; i < color.Length; i++) {
                this.colorData[i] = new SerialColor32(color[i]);
            }
        }

        public short[] deserializeDepth() {
            return depthData;
        }

        public Color32[] deserializeColor() {
            Color32[] ret = new Color32[colorData.Length];
            for(int i= 0; i < colorData.Length; i++) {
                ret[i] = colorData[i].deserialize();
            }
            return ret;
        }
    ...
    [Serializable]
    public struct SerialColor32{
        float a, b, g, r;
        public SerialColor32(Color32 color) {
            this.a = color.a;
            this.b = color.b;
            this.g = color.g;
            this.r = color.r;
